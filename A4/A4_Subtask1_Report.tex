...
%  Module A (Custom B+Tree DB) and Module B (MySQL/Flask API)
% =============================================================================
...
\newcommand{\assignmentinfo}{Assignment 4 --- Module A \& Module B}
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  ...
  \item \textbf{Simplicity.} The formula is trivially expressible in both
        Python (Module~A) and SQL (Module~B).
\end{enumerate}
...
\subsection{Module A: Python Routing Function}

Listing~\ref{lst:router} shows the shard routing function implemented in
\texttt{A4/Module~A/shard\_key\_analysis.py}. The same formula drives both
the Module~A (Python B+Tree) and Module~B (MySQL) implementations.

\begin{lstlisting}[style=python, caption={Hash-based shard router (Module A)},
                   label={lst:router}]
NUM_SHARDS = 3
...
The design described here is fully implemented in
\texttt{A4/Module~A/shard\_key\_analysis.py} (Python routing function and
distribution analysis) and
\texttt{A4/Module~B/subtask1\_shard\_analysis.sql} (SQL distribution
...
+-- A4_Subtask1_Report.tex          # This report
+-- Module A/
|   +-- shard_key_analysis.py       # Python routing + distribution analysis
+-- Module B/
